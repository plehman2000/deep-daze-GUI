...
class Imagine(nn.Module):
    ...
    def save_image(self, epoch, iteration, img=None):
       ...
       #sstqdm.write(f'image updated at "./{str(self.filename)}"\n')
    ...
    def forward(self):
        if exists(self.start_image):
            stqdm.write('Preparing with initial image...')
            
            
            optim = DiffGrad(self.model.model.parameters(), lr = self.start_image_lr)
            
            #with st_stdout("success"):
            pbar = stqdm(range(self.start_image_train_iters), desc='iteration') #edited from trange
        ...
        stqdm.write(f'Imagining "{self.textpath}" from the depths of my weights...\n')
